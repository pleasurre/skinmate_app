...
click('save-offer','h1:botox');assert.equal(run('screen'),'hospitals');assert(run("state.saved.includes('h1')"));click('offer-detail','h1:botox');assert.equal(run('screen'),'detail');assert(els.app.innerHTML.includes('hospital-treatment-row'));assert(els.app.innerHTML.includes('detail-header-name'));assert(els.app.innerHTML.includes('hospital-review-compact'));assert(els.app.innerHTML.includes('hospital-event-rail'));assert(els.app.innerHTML.includes('상담 예약하기'));assert(els.app.innerHTML.includes('결제하기'));assert(!els.app.innerHTML.includes('병원 상세</span>'));assert(!els.app.innerHTML.includes('회복 관련 후기'));click('decision-reviews','h1:botox');assert.equal(run('screen'),'hospital-reviews');assert(els.app.innerHTML.includes('추천에 반영된 평가 보기'));
